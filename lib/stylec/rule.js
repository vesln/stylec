...
Rule.prototype.parseOptions = function() {
  ...
  switch (this._option.type) {
    case 'enum':
      ...
      break;
    case 'number':
      this.options[this._option.key] = +this.options[this._option.key];
      error = isNaN(val);
      break;
    case 'string':
...
Rule.prototype.subscribe = function() {
  if (this.disabled()) return;

  Object.keys(this.events).forEach(function(event) {
    this.source.on(event, this[this.events[event]].bind(this));
  }, this);
};
...
Rule.prototype.option = function() {
  return this.options[this._option.key] || this._option.defaults;
};
